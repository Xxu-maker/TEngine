...
### 4. 配置表模块 (CsvConfigModule)

**核心优势**：
- 🚀 **CSV 直读**：策划直接维护 CSV，无需导表工具与生成代码
- 📦 **随包热更**：CSV 是 YooAsset 普通资源，天然参与热更
...
```
Assets/GameScripts
├── GameEntry.cs + Procedure/   // 主程序（Assembly-CSharp，不热更）
...
- **Assembly-CSharp**：主程序集，包含 `GameEntry` 启动器与 `Procedure/` 流程管理，不参与热更
- **GameLogic**：游戏业务逻辑程序集，存放主要玩法逻辑、UI 与 CSV 配置表模块（工程已移除 Luban，不存在 GameProto 程序集）
...
| 📊 **配置表模块** | [3-6-配置表模块](./3-6-配置表模块.md) | CSV 直读配置表系统 |
